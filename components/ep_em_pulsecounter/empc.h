...
enum EmPcInitialMode { EMPC_INITIAL_MODE_CURRENT = 0, EMPC_INITIAL_MODE_VOLTAGE };

enum EmPcSensorModels {
  EMPC_SENSOR_MODEL_HLW8012 = 0,
  EMPC_SENSOR_MODEL_CSE7759,
  EMPC_SENSOR_MODEL_BL0937
};
...
class EmPcComponent : public PollingComponent {
 public:
  EmPcComponent()
  ...
  void set_initial_mode(EmPcInitialMode initial_mode) {
    current_mode_ = initial_mode == EMPC_INITIAL_MODE_CURRENT;
  }
  void set_sensor_model(EmPcSensorModels sensor_model) { sensor_model_ = sensor_model; }
 ...
 protected:
  ...
  EmPcSensorModels sensor_model_{EMPC_SENSOR_MODEL_HLW8012};
